...
Flight delays impact millions of people who use airplanes every day, and delays can cause various problems such as financial loss, frustration, and missing connection flights. By identifying key factors that predict flight delays, many airlines can improve scheduling. By conducting this analysis, I hope to identify ways to increase the efficiency of airports and decrease the problems faced due to flight delays. 

### Research Questions

1) *Does the time of year influence flight delays?*
...
2) *Which airline has the highest and lowest average delays? *
...
3) *Does the distance traveled by the flight affect the delay departure of a flight?*
...
4) *Do flights have more delays if it is a US public holiday, compared to regular days?*
...
5) *How do different weather conditions impact flight delays (like temperature, precipitation, visibility, etc)?*
...
6) *Does the day of the week influence departure delays?*
...
To ensure accuracy of our analysis, I marked or deleted cancelled flights by setting dep_delay to NA, allowing us to handle cancellations in further analysis. I deleted all rows with zero distance or air_time since such observations weren't providing useful patterns. I truncated negative dep_delay to zero to avoid early departures from distorting the delay measures. I trimmed all the variables to remove the impact of outliers. I converted sched_dep_time, dep_time, sched_arr_time, and similar fields from HHMM ints to proper date-time variables, to enable the accurate calculations of wait time and to eliminate the need to group in performing the join with the weather data. Lastly, since the variable plane_age had many missing or idiosyncratic entries which reduced its utility, I omitted it from the majority of our analysis.
...
This exploratory data analysis helped us to determine the relationship between holidays and flight delays, and find if it is a significant factor. From the violin plot in this EDA, I found that the distribution of both US holidays and non-holidays are similar. Both of the spreads also show consistent shorter delays, while there are a few longer delays. Based on the analysis, we can see that holidays do not particularly lead to longer delays. This helped to recognize that even though there are likely not many differences between holiday delays and regular days, further analysis could be useful in finding any differences. I will use the t-test and linear regression to find any further observations. 
...
```{r, echo=FALSE}
# These are some of the US public holidays that could potentially cause flight delays
...
To examine whether flight departure delays differ between U.S. public holidays and regular days, I used both a Welch two-sample t-test and a simple linear regression model, along with a detailed violin plot. The reason for choosing a t-test was because I wanted to compare the average delays of the independent groups (holidays and regular days). Since the variables dep_delay and is_holiday have different sample sizes, I determined the Welch two-sample t-test would be appropriate to gain more insights. I chose a simple linear regression model because it is crucial for finding how much flight delays differ between holidays and regular days. From this, we can also use the p-value to test statistical significance. Furthermore, the R-squared value determines how much variability there is between the delays and the impact holidays have on that departure delay. 
